...
void op_par_loop_save_soln(char const *name, op_set set,
  ...
  op_mpi_halo_exchanges_grouped(set, nargs, args, 2);
  if (set->size > 0) {
    ...
    if ((OP_kernels[0].count==1) || (direct_save_soln_stride_OP2HOST != getSetSizeFromOpArg(&arg0))) {
      ...
      cudaMemcpyToSymbol(direct_save_soln_stride_OP2CONSTANT,&direct_save_soln_stride_OP2HOST,sizeof(int));
    }
